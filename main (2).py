...
# ==========================================
# [4] SEGMENT 10: REACTOR COMMISSIONING
# ==========================================

@bot.tree.command(
    name="setup_plant", 
    description="ADMIN ONLY: Commission the reactor hardware and appoint the Plant Director"
)
@app_commands.describe(
    reactor_type="The physical model of the reactor core",
    director="The user who will lead the facility as Plant Director"
)
@app_commands.choices(reactor_type=[
    app_commands.Choice(name="CANDU (High Stability / Low Power)", value="CANDU"),
    app_commands.Choice(name="BWR (Fast Heating / High Power)", value="BWR"),
    app_commands.Choice(name="PWR (Standard Balanced Model)", value="PWR")
])
@app_commands.checks.has_permissions(administrator=True)
async def setup_plant(interaction: discord.Interaction, reactor_type: app_commands.Choice[str], director: discord.Member):
    rtype = reactor_type.value
    
    # 1. Physics Calibration based on choice
    if rtype == "CANDU":
        bot.plant.core.meltdown_temp = 3500.0
        bot.plant.core.heat_multiplier = 6.5   # Easier to manage
    elif rtype == "BWR":
        bot.plant.core.meltdown_temp = 2800.0
        bot.plant.core.heat_multiplier = 12.0  # Dangerous / Fast
    elif rtype == "PWR":
        bot.plant.core.meltdown_temp = 3000.0
        bot.plant.core.heat_multiplier = 8.5   # Standard

    # 2. Appoint the Plant Director (PD)
    # We update their database profile directly to bypass hiring logic
    pd_data = bot.plant.db.get_user(director.id)
    pd_data["dept"] = "Management"
    pd_data["rank"] = "Plant Director"
    
    # Ensure they have the XP required for the rank (The Pig Clause Threshold)
    if pd_data["xp"] < 60000:
        pd_data["xp"] = 60000 
    
    # 3. Log Global Stats
    bot.plant.db.data["global_stats"]["active_reactor"] = rtype
    bot.plant.db.save()

    # 4. Success Embed
    embed = discord.Embed(
        title=" FACILITY COMMISSIONING COMPLETE", 
        color=0x2ecc71,
        description=f"The facility is now running on the **{rtype}** architecture."
    )
    embed.add_field(name="Appointed Director", value=director.mention, inline=True)
    embed.add_field(name="Thermal Limit", value=f"{bot.plant.core.meltdown_temp}°C", inline=True)
    embed.add_field(name="Authority", value="Full Administrative Override Granted", inline=False)
    embed.set_footer(text="V3 Systems Online | All core variables synchronized.")
    
    await interaction.response.send_message(embed=embed)

# --- ADMIN ERROR HANDLING ---
@setup_plant.error
async def setup_plant_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message(
            " **Access Denied:** Only a Server Admin or Owner can use this.", 
            ephemeral=True
        )

# ==========================================
# [5] THE REACTOR SYSTEM (Thermal & Integrity)
# ==========================================
class ReactorCore:
    ...
    def calculate(self):
        # 1. Heat Generation: Scales based on the Reactor Type selected in /setup_plant
        # Uses 8.5 as a fallback if the plant hasn't been commissioned yet.
        h_mult = getattr(self, 'heat_multiplier', 8.5)
        fission_heat = self.rods * h_mult
        ...
        # 4. Dynamic Integrity Decay
        # Melts faster if you exceed the specific limit of your chosen reactor type
        limit = getattr(self, 'meltdown_temp', 3000.0)
        
        if self.temp > limit:
            self.integrity -= 2.5  # Critical Meltdown
        elif self.temp > (limit * 0.75):
            self.integrity -= 0.5  # High-heat structural stress

        

# ==========================================
# [6] THE COOLING LOOP (Hydraulics & Pumps)
# ==========================================
class CoolingSystem:
    def __init__(self, plant):
        self.plant = plant
...
# ==========================================
# [7] THE ELECTRICAL BUS (Power Grid Logic)
# ==========================================
class PowerBus:
    def __init__(self, plant):
        self.plant = plant
...
# ==========================================
# [8] THE TURBINE SYSTEM (Steam Energy)
# ==========================================
class TurbineSystem:
    def __init__(self, plant):
        self.plant = plant
...
plant_system.turbines = TurbineSystem(plant_system)
      # ==========================================
# [9] THE LOGISTICS FLEET (Fuel & Repairs)
# ==========================================
...
# ==========================================
# [10] SECURITY & FACILITY CLEARANCE
# ==========================================
...
# ==========================================
# [13] UTILITY: AUTHENTICATION CHECK
# ==========================================
def has_clearance(interaction: discord.Interaction, required_dept: str):
  ...
  # ==========================================
# [14] EMPLOYMENT & PROFILE COMMANDS
# ==========================================
...
async def hire(interaction: discord.Interaction, member: discord.Member):
  ...
  # ==========================================
# [15] AUTHORITY & DISCIPLINARY ACTIONS
# ==========================================
...
async def resign(interaction: discord.Interaction):
  ...
  # ==========================================
# [16] THE MCR DASHBOARD (Global Overview)
# ==========================================
...
# ==========================================
# [17] OPERATIONS PANEL (Rods & Turbines)
# ==========================================
...
# ==========================================
# [18] MAINTENANCE PANEL (The 4 Pumps)
# ==========================================
...
async def maint_panel(interaction: discord.Interaction):
  ...
  # ==========================================
# [19] ELECTRICAL PANEL (EOGs & Grid Mode)
# ==========================================
...
# ==========================================
# [20] LOGISTICS PANEL (Fuel & Supply Chains)
# ==========================================
...
# ==========================================
# [21] THE WORK COMMAND (XP & Promotion Loop)
# ==========================================
